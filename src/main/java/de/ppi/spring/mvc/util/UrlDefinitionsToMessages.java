...
public class UrlDefinitionsToMessages {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(
            UrlDefinitionsToMessages.class);
                    ...
                    try {
                        if (Modifier.isPublic(field.getModifiers())) {
                            if (field.getName().startsWith(PRAEFIX_PARAMETER)) {
                                final ParamFormat format = field.getAnnotation(
                                        ParamFormat.class);
                                final String paramName = field.get(null)
                                        .toString();
                                ...
                                if (format != null) {
                                    formatDefinition.put(paramName, format
                                            .value());
                                }
                            }
                        }
        ...
        for (Class<?> class1 : classes) {
            if (Modifier.isInterface(class1.getModifiers())) {
                ...
                addConstantInfosFromClass(prefix + "." + class1
                        .getSimpleName(), class1.getDeclaredClasses());
     ...
     */
    private void addFieldInformation(String prefix, Map<String,
            String> formatDefinition, Field field) {
        try {
            if (prefix.startsWith("url")) {
                if (!field.getName().startsWith(PRAEFIX_PARAMETER) && !field
                        .getName().startsWith(PRAEFIX_PARAMETER_GROUP)) {
                    final String keyName = createKey(prefix, field
                            .getDeclaringClass().getSimpleName(), field
                            .getName());
                    final String urlValue = createUrl(field.get(null)
                ...
                if (field.getName().startsWith(PRAEFIX_PARAMETER)) {
                    ...
                    MESSAGE_SOURCE_PRAEFIX_PARAMETER_GROUP)) {
                if (field.getName().startsWith(PRAEFIX_PARAMETER_GROUP)) {
                    final String keyName = createKey(prefix, field
                            .getDeclaringClass().getSimpleName(), field
                            .getName().substring(3));
                    final String pgValue = createParamGroup(field.get(null)
                            .toString(), formatDefinition);
     ...
     */
    private String createUrl(String urlAsString, Map<String,
     ...
     */
    private String createParamGroup(String definitionOfParams, Map<String,
            String> formatDefinition) {
        final StringBuilder result = new StringBuilder(2 * definitionOfParams
                .length());
        final StringTokenizer tokens = new StringTokenizer(definitionOfParams,
                ",");
        int i = 0;
        while (tokens.hasMoreTokens()) {
            ...
            result.append(key).append("={").append(i).append(format).append(
                    "},");
            i++;
        }
     ...
     */
    private String createKey(String prefix, String className,
            String fieldName) {
