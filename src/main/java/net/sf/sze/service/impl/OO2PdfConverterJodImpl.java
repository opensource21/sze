...
public class  OO2PdfConverterJodImpl implements OO2PdfConverter {
    ...
    private final Logger log = LoggerFactory.getLogger(OO2PdfConverterJodImpl.class.getName());
    ...
    public OO2PdfConverterJodImpl(File ooCommandFile, File userEnv, int ooPort) {
        super();
        log.info("Creating OO2PdfConverterJodImpl with " + 
                ooCommandFile.getAbsolutePath() + ":" + ooPort + " in " +
                userEnv); 
    ...
    public synchronized void convert(File sourceFile, File outputFile) {
        ...
        try {
            ...
            log.error("Fehler beim Konvertieren von " + 
                    sourceFile.getAbsolutePath() + " -> " +
                    outputFile.getAbsolutePath(), rE);
            closeConnection();
            convertInternal(sourceFile, outputFile);
        }
    }
    ...
    private void ensureStartedOpenOfficeService(boolean init) {        
        try {
            ...
            try {
                ...
                log.warn("Wait for beim Start von OpenOffice unterbrochen.", ie);
            }
            connection = new SocketOpenOfficeConnection("127.0.0.1", ooPort);
            converter = new OpenOfficeDocumentConverter(connection);
            ...
            if (!connected) {
                log.error("Cant connect to OpenOffice. Is OpenOffice running on port "+ 
                        ooPort +". Please start: soffice -headless -accept="+
                        "\"socket,host=127.0.0.1,port=" + ooPort + ";urp;\" -nofirststartwizard");
                if (!init) {
                    throw new IllegalStateException("Can't connect to openoffice!");
                }
            } else {
